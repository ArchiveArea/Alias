...
class MainClass extends PluginBase implements Listener {
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args) {
		switch ($command->getName()) {
			case "alias":
				if (!isset($args[0])) {
					$sender->sendMessage(TextFormat::RED . "§f•§c Sử Dụng: " . $command->getUsage() . "");
					return true;
				}
				...
				if ($player instanceof Player) {
					...
					$names = implode(', ', $names);
					$sender->sendMessage(TextFormat::GREEN . "§f[§cAlias§f]§a Hiển thị bí danh của: " . $name . "§a...");
					$sender->sendMessage(TextFormat::BLUE . "§f[§cAlias§f]§a " . $names . "");
					return true;
				} else {
					if (!is_file($this->getDataFolder() . "players/lastip/" . $name[0] . "/" . $name . ".yml")) {
						$sender->sendMessage(TextFormat::YELLOW . "§f[§cAlias§f]§c Lỗi: Trình phát không tồn tại!");
						return true;
					} else {
